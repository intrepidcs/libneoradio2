...
fn regenerate_bindings() {
    ...
    let bindings = bindgen::Builder::default()
        // Reproduce the committed file's exact top-of-file ordering so a
        // regenerated `src/ffi.rs` is byte-identical to the committed copy (the
        // `bindgen-sync` CI job diffs the two). bindgen emits its own
        // "automatically generated by rust-bindgen" banner *before* any
        // `raw_line`s, but the committed file has the `#![allow(...)]` lint
        // header on line 1 and the banner on line 2. So disable bindgen's auto
        // banner and emit both header lines via `raw_line` in the committed
        // order; bindgen then appends its usual blank line before the items.
        .disable_header_comment()
        .raw_line(
            "#![allow(non_camel_case_types, non_snake_case, non_upper_case_globals, dead_code)]",
        )
        .raw_line("/* automatically generated by rust-bindgen 0.70.1 */")
